...
def train_step(args, split, epoch, loader, model, loss, update_bn=True, 
                ...
                unnorm_net=(lambda pose:pose), unnorm_tgt=(lambda pose:pose)):
  prior_loss = []
  ...
  for i, (input, target, _, _, _) in enumerate(loader):
    ...
    cr_loss = loss(unnorm_net(output.view(output.shape[0], 
                                           target_var.shape[1], 
                                           target_var.shape[2])), dt=unnorm_tgt(target_var))
...
def train_priors(args, train_loader, model, loss, update_bn, logger, optimizer, epoch, nViews=ref.nViews, threshold = 0.9, unnorm_net=(lambda pose:pose), unnorm_tgt=(lambda pose:pose)):
  return train_step(args, 'train', epoch, train_loader[0], model, loss, update_bn, logger, optimizer, threshold=threshold, unnorm_net=unnorm_net, unnorm_tgt=unnorm_tgt)
